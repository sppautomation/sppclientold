...
from sppclient.sdk import client
import util
...
class MyCLI(click.MultiCommand):
    ...
    def get_command(self, ctx, name):
        try:
            ...
            mod = __import__('sppclient.cli.commands.cmd_' + name, None, None, ['cli'])
        except ImportError:
...
@click.option('--url', envvar='SPP_URL', default='http://localhost:8082', metavar='URL', help='SPP url.')
...
def init_logging():
    fd, logfile = tempfile.mkstemp(suffix='.txt', prefix='sppclient')
    os.close(fd)
